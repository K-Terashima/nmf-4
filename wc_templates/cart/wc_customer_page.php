...
<div id="wc_<?php usces_page_name(); ?>">
	...
	<div class="container">
			...
			<section class="section">
				...
				<h3><?php _e('The member please enter at here.','usces'); ?></h3>
				<form action="<?php usces_url('cart'); ?>" method="post" name="customer_loginform" onKeyDown="if (event.keyCode == 13) {return false;}">
					...
					<input name="customerlogin" type="submit" class="btn pink accent-1 right" value="<?php _e(' Next ', 'usces'); ?>" />
			...
			<section class="section">
				<h3><?php _e('The nonmember please enter at here.','usces'); ?></h3>
				<form action="<?php echo USCES_CART_URL; ?>" method="post" name="customer_form" onKeyDown="if (event.keyCode == 13) {return false;}">
					...
					<?php if( usces_is_membersystem_state() ) : ?>
						<tr>
							...
							<td colspan="2">
								<input class="hidden" value=" " /><input name="customer[password1]" type="password" value="<?php echo esc_attr($usces_entries['customer']['password1']); ?>" autocomplete="off" />
								<br>
								<?php if( $member_regmode != 'editmemberfromcart' ) _e('When you enroll newly, please fill it out.', 'usces'); ?>
							</td>
						</tr>
						<tr>
							<th scope="row"><?php if( $member_regmode == 'editmemberfromcart' ) : ?><em><?php _e('*', 'usces'); ?></em><?php endif; ?><?php _e('Password (confirm)', 'usces'); ?></th>
							<td colspan="2"><input name="customer[password2]" type="password" value="<?php echo esc_attr($usces_entries['customer']['password2']); ?>" /></td>
						</tr>
					<?php endif; ?>
